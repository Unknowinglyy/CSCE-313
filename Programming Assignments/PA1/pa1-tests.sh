...
remake
echo -e "\nVerify that 4 server processes run as children\n"
timeout 5 strace -e trace=execve -f -o trace.tst ./client -p 1 -t 0.004 -e 1 -s 4 >/dev/null 2>&1
counts=$(grep -c 'execve("./server", \["./server", "-m", "256", "-s", "[1-4]"\]' trace.tst)
if [ "$counts" -ne 4 ]; then
...
echo -e "\nTesting :: ./client -p 12 -t 0.016 -e -s 4\n"
VAL=$(awk -F, '$1 == "0.016" { print $2 }' BIMDC/12.csv)
if ./client -p 12 -t 0.016 -e 1 -s 4 | grep -qe ${VAL}; then
...
echo -e "\nTesting :: ./client -p 5 -t 59.996 -e 2 -s 4\n"
VAL=$(awk -F, '$1 == "59.996" { print $3 }' BIMDC/5.csv)
if ./client -p 5 -t 59.996 -e 2 -s 4 | grep -qe ${VAL}; then
...
echo -e "\nTesting :: ./client -p 9 -s 4; diff -sqwB <(head -n 1000 BIMDC/9.csv) received/x1.csv\n"
./client -p 9 -s 4 >/dev/null 2>&1
...
##Multiple datapoint test (1)
echo -e "\nTesting :: ./client -p 5 -s 4; diff -sqwB <(head -n 1000 BIMDC/5.csv) received/x1.csv\n"
./client -p 5 -s 4 >/dev/null 2>&1
if test -f "received/x1.csv"; then
...
echo -e "\nTesting :: ./client -s 4 -f 8.csv; diff BIMDC/8.csv received/8.csv\n"
./client -s 4 -f 8.csv >/dev/null 2>&1
...
echo -e "\nTesting :: ./client -s 4 -f 2.csv; diff BIMDC/2.csv received/2.csv\n"
./client -s 4 -f 2.csv >/dev/null 2>&1
...
echo -e "\nTesting :: ./client -s 4 -f 11.csv -m 512; diff BIMDC/11.csv received/11.csv\n"
./client -s 4 -f 11.csv -m 512 >/dev/null 2>&1
