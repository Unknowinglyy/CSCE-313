...
void send_raw_ip_packet(ipheader *ip)
{
    ...
    // Set socket options
    if (setsockopt(sock_fd, IPPROTO_IP, IP_HDRINCL, &enable, sizeof(enable)) == -1) {
    ...
    dest_info.sin_port = htons(udp->udp_dport);

    // Send the packet
    printf("Sending packet...\n");
    if (sendto(sock_fd, ip, ntohs(ip->iph_len), 0, (sockaddr *)&dest_info, sizeof(dest_info)) < 0)
    {
        perror("sendto failed");
        exit(-1);
    }
    //print "(source ip) -->(dest ip)"
    char src_ip[INET_ADDRSTRLEN];
    char dest_ip[INET_ADDRSTRLEN];

    strncpy(src_ip, inet_ntoa(ip->iph_sourceip), INET_ADDRSTRLEN);
    strncpy(dest_ip, inet_ntoa(ip->iph_destip), INET_ADDRSTRLEN);
