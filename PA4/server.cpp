...
/*
MOST OF CODE GOTTEN FROM TA
JUST MODIFIED TO FIT THE ASSIGNMENT!!!
*/
...
int main (int argc, char* argv[]) {
	...
	while ((opt = getopt(argc, argv, "m:r:")) != -1) {
		switch (opt) {
			case 'm':
				...
				break;
			//mandatory argument (has default value tho)
			case 'r':
				r = optarg;
				break;
		}
	...
	//open socket with address="" and r from CLI
	//remember, this is constructor is for the server side
	TCPRequestChannel* control_channel = new TCPRequestChannel("", r);
	//setup for return value on accept function
	int socketFD;
	while (true) {
		//calling TCPReqChan::accept_conn()
		socketFD = control_channel->accept_conn();
		//2nd TCP constructor (one with int sockfd)
		TCPRequestChannel* data_channel = new TCPRequestChannel(socketFD); //creates new channel for each client
		//dispatching handle_process_loop thread for new channel
		thread t(handle_process_loop, data_channel);
		//detach the thread so it can run in the background
		t.detach();
	}
	//gonna have to CTRL+C to shutdown the server
